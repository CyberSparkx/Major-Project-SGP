...
// Use 'var' to ensure hoisting works correctly with jest.mock.
...
jest.mock('@google/generative-ai', () => ({
  GoogleGenerativeAI: jest.fn().mockImplementation(() => ({
    // Use a wrapper to avoid 'undefined' during hoisting
...
describe('POST /api/iot/analyze', () => {
  ...
  it('returns AI analysis result on valid input', async () => {
    ...
    expect(data.result).toBe(
      'This image shows a React component with useState hook.'
    );
  });
